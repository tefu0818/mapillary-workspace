...
def download_image(image_data: dict, save_dir: Path, resolution: int = 1024, chunk_size: int = 8192) -> dict:
    try:
        image_id = image_data['properties']['id']
        image_url = mly.controller.image.get_image_thumbnail_controller(image_id=image_id, resolution=resolution)
...
def download_images(save_dir: Path, access_token: str, west: float, south: float, east: float, north: float, image_type: str = 'flat', compass_angle: tuple = (0, 360)) -> None:
    ...
    for image_data in tqdm(data['features'], desc="Downloading images"):
        result = download_image(image_data, save_dir)
        if result:
...
def main() -> None:
    ...
    """
    args = parse_args()
    download_images(args.save_dir, args.access_token, args.west, args.south, args.east, args.north)
